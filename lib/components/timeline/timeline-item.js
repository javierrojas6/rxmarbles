...
function getPausable$(pause$, obsv$) {
  return pause$.switchMap(pause => pause ? obsv$ : Observable.never());
}
...
function intent(elementClass, DOMSource, isDraggable$) {
  ...
  const isHighlighted$ = getPausable$(isDraggable$, Observable.merge(startHighlight$, stopHighlight$)).startWith(false).publishReplay(1).refCount();
  const timeChange$ = element.events('mousedown').map(path(['currentTarget', 'parentElement'])).map(getPercentageFn).switchMap(getPercentage => mouseMove$.takeUntil(mouseUp$).pluck('pageX').map(getPercentage).distinctUntilChanged());
  return {
    timeChange$,
    isHighlighted$
  };
...
function model(props$, timeSource$, timeChange$, isDraggable$) {
  const restrictedTimeChange$ = getPausable$(isDraggable$, timeChange$).map(max(0)).map(min(100));
  const minChange$ = props$.pluck('minTime').distinctUntilChanged().withLatestFrom(timeSource$, max);
  const maxChange$ = props$.pluck('maxTime').distinctUntilChanged().withLatestFrom(timeSource$, min);
  return Observable.merge( // order matters
