...
import { COLORS, fontBase, userSelectNone, merge as mergeStyles } from '../../styles';
...
function view(sources, value$, isHighlighted$) {
  return Observable.combineLatest(sources.id, sources.content, value$, isHighlighted$).map(([id, content, value, isHighlighted]) => svg.g({
    attrs: {
      class: ELEMENT_CLASS,
...
function OriginalMarble(sources) {
  const {
    DOM,
    time
  } = timelineItem(ELEMENT_CLASS, view, sources);
  const data$ = Observable.combineLatest(time, sources.id).map(([time, id]) => ({
    time,
    id
  }));
  return {
