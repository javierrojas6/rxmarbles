...
export function Sandbox({
  ...
  .distinctUntilChanged().map(exampleKey => examples[exampleKey]).publishReplay(1).refCount();
  const inputStores$ = example$.switchMap(example => store.pluck('inputs').filter(identity) // bug: For some reason inputDataList$ emits old value after
  ...
  const inputDataList$ = Collection.pluck(inputTimelines$, prop('data')).filter(length).debounceTime(0).withLatestFrom(inputStores$, zip).map(map(apply(flip(merge))));
  const vtree$ = Observable.combineLatest(inputDOMs$, outputTimeline.DOM, example$).map(([inputsDOMs, outputDOM, example]) => div({
    style: sandboxStyle
  }, [...inputsDOMs, renderOperatorBox(example.label), outputDOM]));
  return {
    DOM: vtree$,
