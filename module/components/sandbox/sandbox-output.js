...
const toVTStream = curry(function _toVTStream(scheduler, data) {
  ...
  return marbleStreams$.takeUntil(Observable.timer(data.end.time + 1, scheduler));
});
...
function outputStreamToMarbles$(scheduler, stream) {
  ...
  }).takeUntil(stop$).reduce((a, b) => a.concat(b), []).map(items => items.map((item, i) => merge(item, {
    itemId: i
...
//# sourceMappingURL=sandbox-output.js.map