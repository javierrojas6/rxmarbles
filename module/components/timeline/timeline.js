...
function sortMarbleDoms$(marbles$) {
  ...
  return Observable.combineLatest(doms$, dataList$, zip).map(sortBy(path([1, 'time']))).map(map(prop(0)));
}
...
function OriginalTimeline(sources) {
  ...
  const endMarkerProps$ = Observable.combineLatest(marblesState$, end$).map(([marbles, end]) => {
  ...
  const marbles$ = Collection.gather(Marble, marblesSources, marblesState$, 'itemId');
  const marbleDOMs$ = sortMarbleDoms$(marbles$);
  const endMarker = EndMarker(endMarkerSources);
  const vtree$ = Observable.combineLatest(marbleDOMs$, endMarker.DOM).map(([marbleDOMs, endMarkerDOM]) => div({
    style: timelineStyle
  }, [svg({
    attrs: {
      viewBox: '0 0 7 10'
  ...
  const marbleData$ = Collection.pluck(marbles$, prop('data')).debounceTime(0).withLatestFrom(marblesState$, zip).map(map(apply(flip(merge))));
  const data$ = Observable.combineLatest(marbleData$, endMarker.time).map(([marbles, endMarkerTime]) => ({
    marbles,
    end: {
      time: endMarkerTime
    }
