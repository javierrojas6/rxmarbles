...
const sandboxStyle = mergeStyles(bgWhite, elevation1, {
  borderRadius: "2px"
});
...
const render = ({
  ...
  const timelineInputs = operatorObs.switchMap(example => store.pluck("inputs").filter(identity) // bug: For some reason inputDataList$ emits old value after
  // route change. Skip it.
  .skip(1).startWith(inputsToTimelines(example.inputs))).publishReplay(1).refCount();
  const inputTimelines = Collection.gather(Timeline, {
    DOM
  }, timelineInputs, "id").publishReplay(1).refCount();
  const inputDOMs = Collection.pluck(inputTimelines, prop("DOM"));
  const inputDataList = Collection.pluck(inputTimelines, prop("data")).filter(length).debounceTime(0).withLatestFrom(timelineInputs, zip).map(map(apply(flip(merge))));
  const outputTimeline = createOutputStream$(operatorObs, inputDataList); // rendering process
  ...
  const renderedDOM = Observable.combineLatest(inputDOMs, outputTimeline2.DOM).map(([inputsDOMs, outputDOM]) => div({
    style: sandboxStyle
  }, [...inputsDOMs, renderOperatorBox(operator.label), outputDOM]));
  return {
    DOM: renderedDOM,
